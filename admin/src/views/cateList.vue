    ...
    <el-dialog v-model="centerDialogVisible" title="分类" width="30%" center>
        ...
        >
            <el-form-item label="分类名称:" prop="name">
                <el-input v-model="centerAddForm.name" placeholder="请输入分类名称"/>
            </el-form-item>
        </el-form>
        <template #footer>
...
import { getCateList, createCate, updateCateById, delCateById} from '@/api/api'
    ...
    const onAdd = () => {
        centerAddRef.value.validate(async valid => {
            if(valid){
                if(centerAddForm.id != ""){
                    const res = await updateCateById(centerAddForm)
                    if(res){
                    ...
                    const res = await createCate(centerAddForm)
                    if(res){
    ...
    const cateDelete = (row) => {
        ElMessageBox.confirm(
            ...
            const res = await delCateById({id: row.ID})
            if(res){
