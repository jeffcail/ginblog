        ...
        >
            ...
            >
                <template v-slot ="{ row }" v-if="item.prop === 'action'">
                    ...
                    <span style="color: #01AAED;cursor:pointer;" @click="cateDelete(row)">删除</span>
                </template>
            </el-table-column>
        </el-table>
    </div>
    <!-- 新增 -->
    <el-dialog v-model="centerDialogVisible" title="分类" width="30%" center>
        <el-form
            ref="centerAddRef"
            :model="centerAddForm"
            status-icon
        ...
        >
            <el-form-item label="名称:" prop="name">
                <el-input v-model="centerAddForm.name" placeholder="请输入名称"/>
            </el-form-item>
            <el-form-item label="友联地址:" prop="url">
                <el-input v-model="centerAddForm.url" placeholder="请输入友联地址"/>
            </el-form-item>
        </el-form>
        <template #footer>
...
import { getLinkList, createLink, updateLinkById, delLinkById} from '@/api/api'
    ...
    const onAdd = () => {
        centerAddRef.value.validate(async valid => {
            if(valid){
                if(centerAddForm.id != ""){
                    const res = await updateLinkById(centerAddForm)
                    if(res){
                    ...
                    const res = await createLink(centerAddForm)
                    if(res){
    ...
    const cateDelete = (row) => {
        ElMessageBox.confirm(
            ...
            const res = await delLinkById({id: row.ID})
            if(res){
